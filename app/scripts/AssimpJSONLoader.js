...
/* SLIGHTLY MODIFIED BY RCS FOR THIS APPLICATION TO USE A MATERIAL THAT I PASS IN */
/* I MIGHT BE ABLE TO DO THIS ON THE OTHER SIDE BUT I WAS HAVING PROBS */
...
THREE.AssimpJSONLoader.prototype = {
    ...
    /* RCS: HARDCODE PASSING IN A MATERIAL */
    load: function ( url, material, onLoad) {
        ...
        loader.load( url, function ( text ) {
            var scene = scope.parse( JSON.parse( text ), material ); //pass the material so that it doesn't look in the json
            onLoad( scene );
        } );
    },
    ...
    /* RCS: HARDCODE PASSING ALONG THE MATERIAL */
    parse: function ( json, material ) {
        var meshes = this.parseList ( json.meshes, this.parseMesh );
        // var materials = this.parseList ( json.materials, this.parseMaterial );
        return this.parseObject( json, json.rootnode, meshes, material ); // pass along the material
    },
    ...
    /* RCS: Just hardcode passing the material */
    parseObject : function(json, node, meshes, material) {
       ...
       */
        ...
        for(i = 0; node.children && i < node.children.length; ++i) {
            obj.add(this.parseObject(json, node.children[i], meshes, material));
        }
